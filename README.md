...
- **신뢰도 대조** — 강남→서면, 데드라인 **17:00**에서 최대로 벌어집니다:
  조민재·현민채 92.3% vs 김성민·김나은 54.4% (**37.8%p**)
- **지연 장면** — 같은 구간 데드라인 **17:50**:
  탑재 98.7% → 12분 지연 77.6% (흡수) → **25분 지연 0%** (붕괴) → 역 수령 전환 **94.1%** (회복)
- 위 수치는 정시율을 **전일 운행 실적으로 실측**해 쓰기 때문에 날마다 소폭 달라집니다
  (시연 당일 `DEMO_TIME=12:00`으로 위 두 장면을 다시 뽑아 확인하는 것을 권합니다)
...
| **한국철도공사 열차운행정보** | 열차가 늦을 **확률**과 **늦는 크기** — 전일 계획·실적 전수 대조로 정시율·지연 평균을 실측 (기동 시 수집, 하루 캐시) | 어제까지의 실적입니다. 수집 실패 시 공시 정시율로 폴백합니다 |
| **한국천문연구원 특일 정보** | 오늘이 공휴일인지 — 공휴일엔 통근 동선이 사라져 시민 운반 **매칭 리스크 가정을 상향**하고 홈에 배지를 띄웁니다 | 리스크 계수 자체는 가정값입니다 |
